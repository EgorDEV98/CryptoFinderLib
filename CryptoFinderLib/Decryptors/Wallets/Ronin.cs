...
public class Ronin : Decryptor
{
    private const string pattern = @"({\\""data\\""[0-9a-zA-z+.\\""=,'\\/:]+})";
    ...
    public Ronin(InputParameter parameter) : base(parameter)
    {
    ...
    {
        if (_decryptedString == null) return null;
        try
        {
            string outOfJson = Regex
                .Unescape(Regex.Unescape(_decryptedString))
                .Replace("\"{", "{")
                .Replace("}\"", "}");
            
            var jObject = JObject.Parse(outOfJson);
